...
public class ThreadLocalDistributorTest {
	...
	public void testAfterPropertiesSet() throws Exception {
		...
		try (LogCapture logCapture = LoggerTestUtil.configureJDKLogger(
			...
			Assert.assertEquals(logEntrys.toString(), 3, logEntrys.size());

			LogEntry logEntry1 = logEntrys.get(0);

			Assert.assertEquals(
				"_nonstatic is not a static ThreadLocal",
				logEntry1.getMessage());

			LogEntry logEntry2 = logEntrys.get(1);

			Assert.assertEquals(
				"_nullValue is not initialized", logEntry2.getMessage());

			LogEntry logEntry3 = logEntrys.get(2);
