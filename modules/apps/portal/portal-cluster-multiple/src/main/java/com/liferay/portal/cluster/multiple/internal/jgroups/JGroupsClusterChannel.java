	...
	public JGroupsClusterChannel(
		...
		try {
			...
			if (_log.isInfoEnabled()) {
				...
				String propertyListExcludes = StringUtil.merge(
					clusterExecutorConfiguration.propertyListExcludes(),
					StringPool.PIPE);

				String propertyListExcludesRegEx =
					"((" + propertyListExcludes + ")=)(?:\\w*)(;|\\))";

				_log.info(
					"Create a new JGroups channel with properties " +
						jChannelProperties.replaceAll(
							propertyListExcludesRegEx, "$1*$3"));
			}
		}
		catch (Exception e) {
